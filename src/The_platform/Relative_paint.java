	...
	public Relative_paint(ListView<String> duan_text, int para_id, List<List<List<String>>> pian_relative,
		...
		if (pian_relative.get(para_id).size() > 0) {
			...
			for (int i = 0; i < pian_relative.get(para_id).size(); i++) {
				...
				j++;
				Text text2 = new Text(pian_relative.get(para_id).get(i).get(7));
				text2.setFont(Font.font(15));
				gridPane.add(text2, j, i + 1);
				j++;
				String relations = pian_relative.get(para_id).get(i).get(8);
				...
				String center = pian_relative.get(para_id).get(i).get(1);
				...
				auto_ziju[pair1]++;
				auto_ziju[pair2]++;
				String words = pian_relative.get(para_id).get(i).get(6);
				Text link_words = new Text(centerX - 10, ziju_number[0].getLayoutY() - radiusX / 2 - 20, words);
				...
				if (center.equals("1")) {
					...
					if (center.equals("2")) {
						...
						if (center.equals("3")) {
							Circle circle1 = new Circle(centerX + 3 - fix_radiusX, ziju_number[pair1].getLayoutY() - 20,
									2);
							pane.getChildren().addAll(circle1);
							Circle circle2 = new Circle(centerX + 3 + fix_radiusX, ziju_number[pair1].getLayoutY() - 20,
									2);
